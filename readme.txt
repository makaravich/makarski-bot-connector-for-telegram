...
* The "Enable bot" toggle now also gates OUTGOING messages: while the bot is disabled, all user-facing API calls (send*/edit*/delete*/forward*/copy*/answer*/pin*) are suppressed and return `ok = false` with a distinguishable `tgbot_disabled = true` property; administrative calls (getMe, webhook management, getUpdates, setMyCommands, refunds) keep working. New helper `tgbot_can_send()` (filterable via `tgbot_can_send`) is the single source of truth for consumer plugins with their own sending paths.
